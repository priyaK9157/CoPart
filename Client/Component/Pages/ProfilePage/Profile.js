...
const Profile = () => {
    ...
    const findProfileByEmail = async () => {
        try {
            ...
            console.log("abhi ka res", res)
            const email = res.data.Email;
            console.log("email abhi ka", email)
    ...
    return (
        <ScrollView style={tw``}>
            ...
            {fontsLoaded && profile ? (
                ...
                <View style={tw`flex flex-row pt-7 items-center`}>
                        <Text style={[tw`text-3xl mr-2 border border-gray-400 rounded-full pt-4 pb-4 px-4 text-green-500  `, { fontFamily: "MadimiOne" }]}>{profile.name.split(' ').map(name => name.charAt(0)).join('')}</Text>
                        <Text style={[tw`text-2xl mr-2 border-t border-gray-400 pb-4 pt-4 `, { fontFamily: "MadimiOne" }]}>{profile.name}</Text>
                        <TouchableOpacity onPress={navigateToEditProfileName}>
